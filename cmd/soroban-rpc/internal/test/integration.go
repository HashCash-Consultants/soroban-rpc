...
const (
	...
	hcnetCorePort             = 11626
	hcnetCoreArchiveHost      = "localhost:1570"
	goModFile                   = "go.mod"
	goMonorepoGithubPath        = "github.com/HashCash-Consultants/go"
...
func NewTest(t *testing.T, cfg *TestConfig) *Test {
	...
	proxy := httputil.NewSingleHostReverseProxy(&url.URL{Scheme: "http", Host: hcnetCoreArchiveHost})
...
func findDockerComposePath() string {
	...
	if gopath := os.Getenv("GOPATH"); gopath != "" {
		monorepo := filepath.Join(gopath, "src", "github.com", "hcnet", "soroban-rpc")
